...
```
Frontend (Vue 3 SPA)  --HTTP-->  Business Backend (FastAPI, port 8000)  --HTTP-->  Algorithm Backend(s) (ports 9001/9002/9003/9004)
```
...
- **Algorithm Backends**: Standalone services for image (9001), point cloud (9002), mesh (9003), 3DGS (9004) — current repo has mock implementations
...
backend/
  ...
  schemas/
    auth.py, image.py, pointcloud.py, mesh.py, gs.py — Pydantic request/response models
...
mock_algo_gs_service/      — Mock 3DGS algo (synthetic PLY Gaussian sphere)
```
...
| `ALGO_GS_BASE_URL` | `http://127.0.0.1:9004` | 3DGS algo service |
