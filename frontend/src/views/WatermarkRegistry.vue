<template>
  <div class="p-8">
    <div class="mb-6">
      <h2 class="text-2xl font-bold text-gray-800">水印注册库</h2>
      <p class="text-sm text-gray-500 mt-1">浏览所有已嵌入的水印记录</p>
    </div>
    ...
    <div class="bg-white rounded-2xl p-6 mb-6 shadow-sm">
      <div class="flex flex-wrap gap-4 items-center">
        ...
        <el-input v-model="keyword" placeholder="搜索水印值" clearable class="w-48" @keyup.enter="handleSearch">
      ...
      <el-table :data="records" stripe v-loading="loading" class="w-full">
        ...
        <el-table-column prop="watermark_bits" label="水印值" width="140">
          <template #default="{ row }">
            <el-tag size="small" class="font-mono">{{ row.watermark_bits }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="username" label="生成用户" width="110" />
...
const handleSearch = async () => {
  loading.value = true
  try {
    const params = { page: page.value, size: size.value, operation_type: 'embed' }
